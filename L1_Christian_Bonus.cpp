...
int main()
{
	...
	unsigned int longest_name = 0;		//Note, compiler didn't like that it wasn't unsigned in comparison. 
						//Changed to appease comipler.
	...
	for (int i = 0; i < ITERATION; i++)
		{
			cout << left << setw(3) << place_names[i] <<": "<< left << setw(longest_name + 3) 
			<< names[i] + " "<< right << setw(5) << times[i] << " seconds" << endl;
		}
	return 0;
}
...
{
	//Conditional terny operator to return true or false
	return (run_time < 0 || cin.fail()) ? false : true; //cin.fail() will check if it's 
							    //entered as a double, or at least a digit.
}
